...
//$sql="SELECT * from `".$dept_name."_honours`,student_tran where  board='wbchse' and order by merit_point DESC";
    $sql="SELECT serial_no,p.student_name,gender,p.board,f.year_of_passing,p.aggr,p.category,p.ph from `".$dept_name."_honours` as p,`student_tran` as s ,main as t,auth as f where p.board='wbchse' and s.subject_id=p.subject_id and s.student_id=t.student_id and s.student_id=f.student_id and p.ph='yes' order by merit_point DESC";

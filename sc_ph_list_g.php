...
//$sql="SELECT * from `".$dept_name."` where `ph`='yes' and `board`='wbchse' and `category`='ST' order by aggr DESC";
    $sql="SELECT * from `".$dept_name."_honours` as p,`student_tran` as s ,main as t,auth as f where p.`ph`='yes'and p.`category`='SC'and p.board='wbchse' and s.subject_id=p.subject_id and s.student_id=t.student_id and s.student_id=f.student_id order by merit_point DESC";
